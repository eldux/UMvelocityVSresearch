...
for window_size = window_sizes
    ...
    folder_name = "Trained models/MLP_LSTM_w" + num2str(window_size) + "/"
    mkdir(folder_name)
    ...
    lstm_units = optimizableVariable('lstm_units',[1, 256],'Type','integer');
    fc1_units = optimizableVariable('fc1_units',[4, 256],'Type','integer');
    fc2_units = optimizableVariable('fc2_units',[4, 256],'Type','integer');

    fun = @(params)train_MLP_LSTM(XTrain_w, YTrain_w, XValid_w, YValid_w, params, folder_name, window_size);
    results = bayesopt(fun,[lstm_units, fc1_units, fc2_units],'Verbose',1,...
