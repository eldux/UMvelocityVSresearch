...
for window_size = window_sizes
    ...
    folder_name = "Trained models/MLP_3layers_w" + num2str(window_size) + "/"
    mkdir(folder_name)
    ...
    fc1_units = optimizableVariable('fc1_units',[8, 1024],'Type','integer');
    fc2_units = optimizableVariable('fc2_units',[8, 1024],'Type','integer');
    fc3_units = optimizableVariable('fc3_units',[8, 1024],'Type','integer');

    fun = @(params)train_MLP_3layers(XTrain4D, YTrain4D, XValid4D, YValid4D, params, folder_name, window_size);
    results = bayesopt(fun,[fc1_units fc2_units fc3_units],'Verbose',1,...
